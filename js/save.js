import { state, SAVE_KEY, clamp } from "./data.js";
...
export function loadGame() {
  try {
    ...
    for (const key of Object.keys(state.inventory)) {
      if (data.inventory && Number.isFinite(data.inventory[key])) state.inventory[key] = Math.max(0, Math.floor(data.inventory[key]));
    }
