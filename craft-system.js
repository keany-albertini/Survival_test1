(()=>{
  ...
  const readQuickbar=()=>{
    try{
      const q=JSON.parse(localStorage.getItem(quickbarKey)||'[]');
  ...
  const bag=document.createElement('div');bag.className='bag-view';
  [...panel.querySelectorAll(':scope > .inv-row')].forEach(r=>bag.appendChild(r));
  panel.insertBefore(bag,tabs.nextSibling);

  const craft=document.createElement('div');craft.className='craft-view';
  craft.innerHTML=`
  ...
  const equipment=document.createElement('section');equipment.className='equipment-pane';
  equipment.innerHTML=`<h3>Équipement</h3><div class="paperdoll"><div class="equip-slot head">TÊTE<strong>—</strong></div><div class="equip-slot chest">TORSE<strong>—</strong></div><div class="equip-slot hand" id="equipHand">MAIN<strong id="equipHandValue">Vide</strong></div><div class="equip-slot legs">JAMBES<strong>—</strong></div><div class="doll-person"><div class="doll-head"></div><div class="doll-body"></div><div class="doll-arm l"></div><div class="doll-arm r"></div><div class="doll-leg l"></div><div class="doll-leg r"></div></div></div>`;
  const content=document.createElement('section');content.className='inventory-content';[heading,tabs,bag,craft].forEach(el=>content.appendChild(el));
  const layout=document.createElement('div');layout.className='inventory-layout';layout.append(equipment,content);panel.appendChild(layout);
  ...
  function ensureToolRows(){
    if(!document.getElementById('invAxe')){const r=document.createElement('div');r.className='tool-row';r.id='rowAxe';r.innerHTML='<div class="tool-main"><span>🪓 Hache primitive</span><button class="equip-btn" data-tool="axe">METTRE BARRE</button></div><b id="invAxe">0</b>';bag.appendChild(r)}
    if(!document.getElementById('invPickaxe')){const r=document.createElement('div');r.className='tool-row';r.id='rowPickaxe';r.innerHTML='<div class="tool-main"><span>⛏️ Pioche primitive</span><button class="equip-btn" data-tool="pickaxe">METTRE BARRE</button></div><b id="invPickaxe">0</b>';bag.appendChild(r)}
  }

  function setEquipped(tool){
    const inv=readInventory();
    if(tool&&!(inv[tool]>0))tool='';
    localStorage.setItem(equippedKey,tool||'');
    updateHeldTool();
  }
  ...
  function updateQuickbar(){
    ...
    quickSlots.forEach((slot,i)=>{
      const item=q[i]&&inv[q[i]]>0?q[i]:'';
      if(q[i]&&!item){q[i]=''}
      slot.classList.toggle('active',!!item&&item===equipped);
      slot.classList.toggle('empty',!item);
      slot.classList.remove('weapon','tool','consumable');
      if(item)slot.classList.add('tool');
      slot.innerHTML=`<span class="num">${i+1}</span>${item?`<span class="item-icon">${itemDefs[item].icon}</span><span class="type">${itemDefs[item].label}</span>`:'<span class="type">VIDE</span>'}`;
      slot.dataset.item=item;
    });
    saveQuickbar(q);
  }

  function placeInQuickbar(tool,select=true){
    const inv=readInventory();if(!(inv[tool]>0))return false;
    const q=readQuickbar();
    let index=q.findIndex(x=>x===tool);
    if(index<0)index=q.findIndex(x=>!x);
    if(index<0){toast('Barre pleine');return false}
    q[index]=tool;saveQuickbar(q);
    if(select)setEquipped(tool);
    updateQuickbar();
    return true;
  }

  function updateHeldTool(){
    const inv=readInventory(),tool=localStorage.getItem(equippedKey)||'',valid=tool&&inv[tool]>0;
    held.className='held-tool';if(valid)held.classList.add('show',tool);
    bag.querySelectorAll('.equip-btn').forEach(btn=>{const inBar=readQuickbar().includes(btn.dataset.tool);btn.textContent=inBar?'DANS LA BARRE':'METTRE BARRE'});
    const hand=document.getElementById('equipHand'),value=document.getElementById('equipHandValue');
    if(hand&&value){hand.classList.toggle('active',!!valid);value.textContent=valid?itemDefs[tool].icon+' '+itemDefs[tool].label:'Vide'}
    updateQuickbar();
  }

  function refreshResourceRows(inv){
    const map={invWood:'wood',invStone:'stone',invWater:'water',invBerries:'berries'};
    Object.entries(map).forEach(([id,key])=>{const el=document.getElementById(id);if(el){el.textContent=inv[key]||0;const row=el.closest('.inv-row');if(row)row.hidden=!(inv[key]>0)}});
  }

  function refresh(){
    ensureToolRows();const inv=readInventory();refreshResourceRows(inv);
    const axe=document.getElementById('invAxe'),pick=document.getElementById('invPickaxe');if(axe)axe.textContent=inv.axe||0;if(pick)pick.textContent=inv.pickaxe||0;
    const rowAxe=document.getElementById('rowAxe'),rowPick=document.getElementById('rowPickaxe');if(rowAxe)rowAxe.hidden=!(inv.axe>0);if(rowPick)rowPick.hidden=!(inv.pickaxe>0);
    craft.querySelectorAll('.craft-card').forEach(card=>{const type=card.dataset.recipe,cost=recipes[type],b=card.querySelector('.craft-btn');b.disabled=inv.wood<cost.wood||inv.stone<cost.stone});
    updateHeldTool();
  }

  function craftTool(type){
    const inv=readInventory(),cost=recipes[type];
    if(!cost)return;
    if(inv.wood<cost.wood||inv.stone<cost.stone){toast('Ressources insuffisantes');refresh();return}
    inv.wood-=cost.wood;inv.stone-=cost.stone;inv[type]=(inv[type]||0)+1;
    saveInventory(inv);
    const q=readQuickbar(),hasFree=q.some(x=>!x),already=q.includes(type);
    if(hasFree&&!already)placeInQuickbar(type,true);
    else if(!localStorage.getItem(equippedKey))setEquipped(type);
    refresh();
    window.refreshInventoryVisibility?.();
    toast(itemDefs[type].full+' fabriquée');
  }
  ...
  quickSlots.forEach((slot,i)=>slot.addEventListener('click',()=>{const q=readQuickbar(),tool=q[i];if(!tool)return;setEquipped(tool);toast(itemDefs[tool].label+' équipée')}));
