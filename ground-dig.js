(()=>{
  ...
  const shovelEquipped=()=>{const inv=readInventory();return equipped()==='shovel'&&(inv.shovel||0)>0&&readQuickbar().includes('shovel')};
  ...
  const style=document.createElement('style');style.textContent=`.dig-action{position:fixed;z-index:31;right:102px;bottom:238px;width:98px;height:46px;border-radius:12px;font-size:9px;padding:6px;color:#fff;border:1px solid #d7c58b88;background:#665032e6;box-shadow:0 3px 10px #0008;font-weight:900}.dig-action[hidden]{display:none}`;document.head.appendChild(style);
  const digBtn=document.createElement('button');digBtn.id='digBtn';digBtn.className='dig-action';digBtn.hidden=true;digBtn.textContent='CREUSER';document.body.appendChild(digBtn);
  function currentBiome(){const parts=(topinfo.textContent||'').split('•').map(x=>x.trim());return parts[1]||''}
  function groundResource(){const b=currentBiome();if(b==='Plage'||b==='Désert')return{key:'sand',label:'Sable'};if(b==='Neige')return{key:'snow',label:'Neige'};if(b==='Eau')return null;return{key:'dirt',label:'Terre'}}
